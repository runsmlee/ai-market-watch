...
export default function AdvancedFilters({ 
  ...
  return (
    <div className="relative mb-4 sm:mb-6">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-white/[0.01] to-transparent rounded-lg"></div>
      
      {/* Main container - 더 미니멀한 스타일 */}
      <div className="relative bg-white/[0.02] border border-white/[0.06] rounded-lg 
                      backdrop-blur-sm overflow-hidden">
        {/* Header - 컴팩트한 디자인 */}
        <div className="flex items-center justify-between gap-2 p-2.5 sm:p-3 border-b border-white/[0.05]">
          <button
          ...
          >
            <div className="w-5 h-5 sm:w-6 sm:h-6 bg-white/[0.06] border border-white/[0.08] 
                           rounded flex items-center justify-center
                           group-hover:bg-white/[0.08] transition-colors duration-200">
              <Filter className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white/70" />
            </div>
            <span>Advanced Filters</span>
            {activeFiltersCount > 0 && (
              <div className="px-1 py-0.5 bg-white/90 text-gray-900 rounded text-[10px] font-semibold min-w-[1rem] text-center">
                {activeFiltersCount}
              </div>
            )}
            <div className="transition-transform duration-200">
              {isCollapsed ? (
                <ChevronDown className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white/50" />
              ) : (
                <ChevronUp className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white/50" />
              )}
            </div>
          </button>
          ...
          {activeFiltersCount > 0 && (
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 
                       ...
                       rounded text-[10px] sm:text-xs font-medium text-white/70 hover:text-white/90
        ...
        <div className={`overflow-hidden transition-all duration-300 ease-out ${
          ...
          <div className="p-2.5 sm:p-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-2.5 sm:gap-3">
              ...
              {/* Text Search - 미니멀한 스타일 */}
              <div className="space-y-1.5">
                <label className="flex items-center gap-1.5 text-[10px] font-medium text-white/70 tracking-wide uppercase">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="Company name, CEO, keywords..."
                    className="w-full px-2.5 py-1.5 sm:px-3 sm:py-2 bg-white/[0.02] border border-white/[0.06] rounded
                             text-xs text-white/90 placeholder-white/40 backdrop-blur-sm
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-0.5">
                  {locations.length === 0 ? (
                    <div className="text-white/40 text-[10px] py-1">No locations available</div>
                  ) : (
              ...
              <div className="space-y-1.5">
                <label className="flex items-center gap-1.5 text-[10px] font-medium text-white/70 tracking-wide uppercase">
                  <Calendar className="w-2.5 h-2.5 text-white/50" />
                  Founded
                </label>
                <div className="space-y-1.5">
                  <div className="relative">
                  ...
                  <div className="flex justify-between text-[10px]">
                    ...
                    <span className="px-1.5 py-0.5 bg-white/[0.06] border border-white/[0.08] 
                                   rounded text-white/90 font-medium text-[10px]">
                      {filters.yearFrom}+
                    </span>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-0.5">
                  {categories.length === 0 ? (
                    <div className="text-white/40 text-[10px] py-1">No categories available</div>
                  ) : (
